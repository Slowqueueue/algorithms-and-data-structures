...
class Tree
{
	...
	{
	public:
		K key;// добавить критерий в узел
		T data;
		T criteria;
		Node* left, * right;
		Node(K k, T d)
...
public:
	...
	void Show(bool foundcrit); //Вертикальная печать дерева
...
private:
	...
	void _Show(Node* r, int level, bool foundcrit); //Вспомогательная функция для вертикальной печати 	
...
template <class K, class T>
void  Tree<K, T>::_Show(typename Tree<K, T>::Node* r, int level, bool foundcrit) {
	if (r == NULL) return;
	_Show(r->right, level + 1, foundcrit);
	for (int i = 0; i < 3 * level; i++) cout << " ";
	if (foundcrit) cout << r->key << "(" << r->criteria << ")" << endl;
	else cout << r->key << endl;
	_Show(r->left, level + 1, foundcrit);
	return;
}
//Вертикальная печать
template <class K, class T>
void Tree<K, T>::Show(bool foundcrit) {
	if (root == NULL) {
		cout << "Дерево пусто" << endl;
		return;
	}
	_Show(root, 0, foundcrit);
}
...
template <class K, class T>
void Tree<K, T>::ReverseIterator::rbeg()
{
	cur = ptr->root;
	while (cur != NULL && cur->right != NULL)
		cur = cur->right;
...
template <class K, class T>
void Tree<K, T>::ReverseIterator::rend()
{
	cur = ptr->root;
	while (cur != NULL && cur->left != NULL)
		cur = cur->left;
