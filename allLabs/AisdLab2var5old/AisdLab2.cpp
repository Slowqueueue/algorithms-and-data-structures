...
void main() {
	Tree<int, int> tree;
	//Tree<int, int> betatree;
	//betatree.Insert(10, 10);
	//Tree<int, int> tree(betatree);
	...
	tree.Insert(10, 10); tree.Insert(4, 4); tree.Insert(9, 9); tree.Insert(3, 3); tree.Insert(5, 5);
	tree.Insert(8, 8); tree.Insert(1, 1); tree.Insert(2, 2); tree.Insert(7, 7); tree.Insert(6, 6); tree.Insert(12, 12);
	while (true)
	{
		...
		case 7:
			...
			if (!useReverseiter && !it.is_off()) it.beg();
			if (useReverseiter && !revit.is_rev_off()) revit.rbeg();
			foundcrit = false;
			break;
		case 8:
