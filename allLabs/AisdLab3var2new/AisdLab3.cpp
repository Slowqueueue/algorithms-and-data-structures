...
void main() {
	...
	while (true)
	{
		...
		case 5:
			...
			try
			{
				cout << AVLtree.Replace(key, value) << endl;
			}
		...
		case 6:
			cin >> key;
			cin >> value;
			cout << AVLtree.Insert(key, value) << endl << endl;
			break;
		case 7:
			cin >> key;
			cout << AVLtree.Delete(key) << endl << endl;
			it = AVLtree.beg();
			revit = AVLtree.rbeg();
			break;
		case 8:
...
void TestEffect::test_rand(int n)
{
	...
	for (int i = 0; i < n; i++)
	{
		...
		AVLtree.Insert(m[i], 1);
	}
	BSTtree.getCountNodesView();
	AVLtree.getCountNodesView();
	...
	for (int i = 0; i < n / 2; i++)
		if (i % 10 == 0)
		{
			...
			AVLtree.Delete(lrand());
			D += BSTtree.getCountNodesView();
			Db += AVLtree.getCountNodesView();
			BSTtree.Insert(m[rand() % n], 1);
			AVLtree.Insert(m[rand() % n], 1);
			I += BSTtree.getCountNodesView();
			Ib += AVLtree.getCountNodesView();
			try
			{
			...
			try
			{
				AVLtree.GetData(m[rand() % n]);
				Sb += AVLtree.getCountNodesView();
			}
			catch (AVLTreeException&) { Sb += AVLtree.getCountNodesView(); };
		}
		else
		{
			int ind = rand() % n;
			BSTtree.Delete(m[ind]);
			AVLtree.Delete(m[ind]);
			D += BSTtree.getCountNodesView();
			Db += AVLtree.getCountNodesView();
			INT_64 key = lrand();
