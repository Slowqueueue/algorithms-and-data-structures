...
public:
	...
	bool Insert(K key, T val);//Включение данных с заданным ключом
	bool Delete(K key); //Удаление данных с заданным ключом
...
private:
	...
	void _Show(Node* r, int level); //Вспомогательная функция для вертикальной печати дерева
	...
	Node* _rotateL(Node* target);
	Node* _rotateR(Node* target);
...
template <class K, class T>
typename Tree<K, T>::Node* Tree<K, T>::_rotateL(typename Tree<K, T>::Node* target)
{
	if (target == nullptr)
		return nullptr;
	else {
...
template <class K, class T>
typename Tree<K, T>::Node* Tree<K, T >::_rotateR(typename Tree<K, T>::Node* target)
{
	if (target == nullptr)
		return nullptr;
	else {
