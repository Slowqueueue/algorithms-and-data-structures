...
void firstTaskMenu(Graph<Vertex<string, int>, Edge<Vertex<string, int>, int, int>>* graph) {
    ...
    cin >> vertexIndex;
    if (vertexIndex >= graph->getVertexCount() || graph->getVertexCount() < 0) {
        cout << "Заданная вершина не существует" << endl;
        return;
    }
    Vertex<string, int>* vertex;
    try {
    ...
    cin >> length;
    FirstTask<Vertex<string, int>, Edge<Vertex<string, int>, int, int>> firstTask(graph, vertex, length);
    int key;
    while (true) {
        cout << endl;
        cout << "Задача для невзвешенного графа" << endl;
...
start:
    ...
    while (exit) {
        ...
        cout << "25.Задача для невзвешенного графа" << endl;
        cout << "26.Установить вес для ребра" << endl;
        cout << "27.Выход" << endl;
        cin >> key;
        switch (key) {
        case 27:
